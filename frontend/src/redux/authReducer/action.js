...
const createAccount = (user) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: types.SIGN_UP_REQUEST_FAILED,
...
const updateUserData = (pic, token) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: types.UPDATE_USER_DATA_REQUEST_SUCCESS,
      payload: "User Data Update Successfully.",
    });
  } catch (error) {
    dispatch({
      type: types.UPDATE_USER_DATA_REQUEST_FAILED,
      payload: error.response.data.error,
    });
  }
};
