...
const getChats = () => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: types.ALL_CHATS_REQUEST_FAIL,
...
const createGroup = (obj) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: types.CREATE_GROUP_REQUEST_SUCCESS,
      payload: result.data,
    });
  } catch (error) {
    console.log(error);
...
const changeGroupName = (obj) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: types.RENAME_GROUP_REQUEST_SUCCESS,
      payload: result.data,
    });
  } catch (error) {
    console.log(error);
...
const setWebSocketReceivedMessage =
  ...
  async (dispatch) => {
    ...
    if (Array.isArray(allMessages) && allMessages.length > 0) {
      const isSameChat = allMessages.some(
        (message) => message.chat._id === receivedMessage.chat._id
      );
      const isAlreadyReceived = allMessages.some(
        (message) => message._id === receivedMessage._id
      );

      if (isSameChat && !isAlreadyReceived) {
        dispatch({
          type: types.WEB_SOCKET_RECEIVED_MESSAGE,
          payload: receivedMessage,
        });
...
const addMembersInGroup = (obj) => async (dispatch) => {
  ...
  try {
    const result = await axios.put(
      `${END_POINT}/api/chat/group/user/add`,
      obj,
      {
        headers: {
          Authorization: jwtToken(),
    ...
    dispatch({
      type: types.ADD_NEW_MEMBER_GROUP_REQUEST_SUCCESS,
      payload: result.data,
    });
  } catch (error) {
    console.log(error);
...
const removeMembersFromGroup = (obj) => async (dispatch) => {
  ...
  try {
    const result = await axios.put(
      `${END_POINT}/api/chat/group/user/remove`,
      obj,
      {
        headers: {
          Authorization: jwtToken(),
    ...
    dispatch({
      type: types.REMOVE_MEMBER_FROM_GROUPP_REQUEST_SUCCESS,
      payload: result.data,
    });
  } catch (error) {
    console.log(error);
